...
function BulletList({ bullets, satTipIdx }: { bullets: string[]; satTipIdx: number }) {
  return (
        ...
        return (
          <div
            ...
            className={`bullet-enter flex gap-3 items-start rounded-xl px-3.5 py-2.5 border-l-4 ${
              isSAT
                ? 'bg-amber-50 border-amber-400 border border-amber-200 border-l-4'
...
function ConceptSlide({ slide, slideCount }: { slide: Slide; slideCount: number }) {
        ...
        <div className="flex-1 flex gap-3 px-5 pt-3 pb-4 min-h-0 overflow-hidden bg-indigo-50/20">
          ...
          {bullets.length > 0 && (
            <div className="flex-1 flex flex-col gap-2 justify-center">
              <BulletList bullets={bullets} satTipIdx={satTipIdx} />
            </div>
          )}
        </div>
      </>
    ...
    return (
      <>
        ...
        <div className="flex-1 flex flex-col px-5 pt-3 pb-4 gap-2 min-h-0 overflow-hidden bg-indigo-50/20">
          ...
          <div
            data-testid="formula-block"
            className="formula-enter shrink-0 bg-white border-2 border-indigo-200 rounded-2xl px-5 py-3 flex items-center justify-center gap-1 shadow-md text-[15px] font-mono"
            style={{ animationDelay: '0.05s' }}
          >
            ...
            <span
              ...
              onMouseLeave={() => setHighlightParam(null)}
            >{k}</span>
            <span className="text-[11px] text-slate-400 ml-2 font-sans">← hover to highlight</span>
          </div>
          <div className="flex-1 flex gap-3 min-h-0">
      ...
      <div className="flex-1 flex flex-col px-5 pt-3 pb-4 gap-3 min-h-0 overflow-hidden bg-amber-50/30">
        ...
        {choices ? (
          <>
              ...
              {choices.map(({ label, value }, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-sm font-semibold text-slate-700 leading-snug">{value}</span>
            ...
            {selected && (
              <p className="answer-pop text-center text-xs font-bold">
          ...
          <div className="flex-1 flex flex-col gap-2 justify-end">
            ...
            {showSolution && content.solution && (
              <div className="step-enter bg-white border-2 border-emerald-300 rounded-2xl px-4 py-3 text-sm text-slate-700 leading-relaxed space-y-1.5">
...
function SolutionSlide({ slide, slideCount }: { slide: Slide; slideCount: number }) {
  const { title, content, index } = slide
  const { variant } = content
  const bullets = content.bullets ?? []
        ...
        <div className="flex-1 flex flex-col justify-center">
          <div className="relative flex flex-col">
            {steps.slice(0, visibleCount).map((step, i) => (
              <div key={i} className="step-enter flex gap-3 relative pb-3">
                ...
                <div className="flex-1 bg-white border border-emerald-100 rounded-xl px-3.5 py-2.5 text-sm text-slate-700 leading-snug shadow-sm">
                  {step}
                </div>
              </div>
            ))}
...
function SummarySlide({ slide, slideCount }: { slide: Slide; slideCount: number }) {
  const { title, content, index } = slide
  const bullets = content.bullets ?? []
      ...
      <div className="flex-1 flex flex-col px-5 pt-4 pb-4 gap-2.5 min-h-0 overflow-hidden bg-violet-50/20">
        <div className="flex-1 flex flex-col justify-center gap-2">
          {summaryItems.map((item, i) => (
            <div
              key={i}
              className="bullet-enter flex gap-3 items-center bg-white border-l-4 border-violet-400 border border-violet-100 rounded-xl px-4 py-2.5 shadow-sm"
              style={{ animationDelay: `${0.08 + i * 0.1}s` }}
            >
        ...
        {practice && (
          ...
          >
            <p className="text-[10px] font-extrabold text-violet-400 uppercase tracking-widest mb-1.5">
              Practice Problem
            </p>
            ...
            {showAnswer && (
              <div className="step-enter mt-2 bg-violet-50 border border-violet-200 rounded-xl px-3 py-2 text-sm font-mono text-violet-900">
...
function CoverSlide({ slide, slideCount }: { slide: Slide; slideCount: number }) {
    ...
    <div className="flex-1 flex flex-row overflow-hidden bg-gradient-to-br from-indigo-800 via-indigo-900 to-violet-950 relative">
      ...
      <div className="flex-1 flex flex-col justify-between px-7 py-5 relative z-10 min-w-0">
        ...
        <div className="flex items-center gap-2">
          <span className="text-indigo-300 text-[9px] font-extrabold tracking-[0.2em] uppercase">SAT Math Prep</span>
          <span className="flex-1 h-px bg-indigo-700/60" />
          <span className="text-indigo-400 text-[9px] font-mono">{slideCount - 1} slides</span>
        </div>
        ...
        <div className="flex flex-col gap-1.5">
          <p className="text-indigo-400 text-[9px] font-extrabold tracking-widest uppercase mb-0.5">
            What you&apos;ll master
          </p>
          {bullets.map((b, i) => (
        ...
        <div className="flex items-center gap-3">
          <span className="text-indigo-500 text-[9px] font-mono">~15 min</span>
          <span className="flex-1 h-px bg-indigo-800" />
          <span className="text-indigo-400 text-[9px] font-extrabold tracking-widest uppercase">Start →</span>
        </div>
      </div>
...
function SlideCard({ slide, slideCount, className, style }: {
  slide: Slide; slideCount: number; className: string; style?: CSSProperties
}) {
  const isCover = slide.content.variant === 'cover'
  const type = isCover ? null : detectSlideType(slide.title)
  return (
    <div className={className} style={style}>
      {isCover                    && <CoverSlide    slide={slide} slideCount={slideCount} />}
      {type === 'concept'         && <ConceptSlide  slide={slide} slideCount={slideCount} />}
      {type === 'question'        && <QuestionSlide slide={slide} slideCount={slideCount} />}
      {type === 'solution'        && <SolutionSlide slide={slide} slideCount={slideCount} />}
      {type === 'summary'         && <SummarySlide  slide={slide} slideCount={slideCount} />}
    </div>
  )
}
...
export function SlidePanel({ slide, slideCount, highlights: _highlights }: Props) {
  ...
  return (
    <div className="flex-1 flex items-start justify-center min-h-0 p-4">
      <div className="relative w-full" style={{ aspectRatio: '4/3' }}>
        ...
        />
        {/* Exiting slide — exit animation overlaid on top */}
        {exitingSlide && (
          <SlideCard
            slide={exitingSlide}
            slideCount={slideCount}
